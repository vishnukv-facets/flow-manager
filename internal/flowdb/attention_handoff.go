...
func CreateAttentionHandoff(db *sql.DB, h AttentionHandoff) (AttentionHandoff, error) {
	if db == nil {
		return AttentionHandoff{}, fmt.Errorf("flowdb: create attention handoff requires db")
	}
	...
	if h.FeedItemID == "" || h.Sender == "" || h.Receiver == "" || h.Context == "" || h.RequestedVerdict == "" || h.RequestedAt == "" || h.ExpiresAt == "" {
		return AttentionHandoff{}, fmt.Errorf("flowdb: attention handoff requires feed_item_id, sender, receiver, context, requested_verdict, requested_at, expires_at")
	}
	if !validHandoffStatus(h.Status) {
		return AttentionHandoff{}, fmt.Errorf("flowdb: unsupported attention handoff status %q", h.Status)
	}
	_, err := db.Exec(
	...
	if err != nil {
		return AttentionHandoff{}, fmt.Errorf("flowdb: create attention handoff: %w", err)
	}
	return h, nil
}
...
func GetAttentionHandoff(db *sql.DB, id string) (AttentionHandoff, error) {
	if db == nil {
		return AttentionHandoff{}, fmt.Errorf("flowdb: get attention handoff requires db")
	}
	...
	if err != nil {
		return AttentionHandoff{}, fmt.Errorf("flowdb: get attention handoff %q: %w", id, err)
	}
...
func LatestAttentionHandoffForFeed(db *sql.DB, feedItemID string) (AttentionHandoff, bool, error) {
	if db == nil {
		return AttentionHandoff{}, false, fmt.Errorf("flowdb: latest attention handoff requires db")
	}
	...
	if err != nil {
		return AttentionHandoff{}, false, fmt.Errorf("flowdb: latest attention handoff for feed %q: %w", feedItemID, err)
	}
	h.Reason = reason.String
	h.RespondedAt = respondedAt.String
	return h, true, nil
...
func RespondAttentionHandoff(db *sql.DB, id, verdict, reason, at string) (AttentionHandoff, error) {
	...
	if reason == "" {
		return AttentionHandoff{}, fmt.Errorf("flowdb: attention handoff response requires a reason")
	}
	...
	if h.Status != "pending" {
		return AttentionHandoff{}, fmt.Errorf("flowdb: attention handoff %s is %s, not pending", id, h.Status)
	}
	res, err := db.Exec(
	...
	if err != nil {
		return AttentionHandoff{}, fmt.Errorf("flowdb: respond attention handoff: %w", err)
	}
	n, _ := res.RowsAffected()
	if n == 0 {
		return AttentionHandoff{}, fmt.Errorf("flowdb: attention handoff %s is no longer pending", id)
	}
...
func ExpireAttentionHandoffs(db *sql.DB, now string) (int, error) {
	if db == nil {
		return 0, fmt.Errorf("flowdb: expire attention handoffs requires db")
	}
	...
	if err != nil {
		return 0, fmt.Errorf("flowdb: expire attention handoffs: %w", err)
	}
	n, _ := res.RowsAffected()
	return int(n), nil
}
...
func handoffVerdictStatus(verdict string) (string, error) {
	...
	case "decline", "declined":
		return "declined", nil
	default:
		return "", fmt.Errorf("flowdb: unsupported attention handoff verdict %q (want accept|decline)", verdict)
	}
}
